export default function BlankProjectWindow({projectList, handleAddProject}){
    let projectIndexString = (projectList.length+1).toString();
    let projectIndex = projectList.length+1;
    let newProject = {
        index:projectIndex, 
        projectName:"New Project "+ projectIndexString,
        date: '',
        description: '',
        tasks: [], 
        saved:false
    ...
    return(
        <div className="flex flex-col items-center justify-center h-full bg-gray-200">
            <div className="flex flex-col items-center space-y-6 p-8 rounded-lg bg-white shadow-sm">
                ...
                <button 
                className="mt-4 px-6 py-3 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-colors shadow-sm hover:shadow-md"
                onClick={() => handleAddProject(newProject)}>
